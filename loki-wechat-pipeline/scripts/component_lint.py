...
CHECKS = [
    (re.compile(r"white-space\s*:\s*pre(?!-wrap|-line)", re.I), "ERROR",
...
def lint_file(path):
    ...
    for m in re.finditer(r"```html\s*\n(.*?)```", text, re.S):
        ...
        for rx, level, msg in CHECKS:
            if rx.search(html):
                add(level, msg)
        # 四周虚线框：quote-box/oneliner-card/占位块豁免
        if FOURSIDE_DASHED.search(html) and not CENTERED.search(html):
            is_quote = bool(re.search(r"border.*dashed.*border-radius.*padding", html, re.I | re.S))
            if not is_quote:
                add("WARN", "四周虚线框 border:…dashed（正文强调请用左竖条；"
                            "quote-box/oneliner-card/居中占位块可用 dashed）")
    return name, found
